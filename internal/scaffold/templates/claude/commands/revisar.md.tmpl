...
Leia a spec `$ARGUMENTS` inteira no cérebro, direto por filesystem. Extraia:
...
- **Vocabulário de processo vazando para artefato entregue:** `grep` por "spec",
  "CA-" e "RF-" nos READMEs e no código de produção tocado. Deve voltar vazio.
...
saber depois. Eles não viram spec sozinhos — vão para o cérebro.
